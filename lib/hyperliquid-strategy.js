...
const { buildResearchLanesStatus, trackedSnapshot: researchLanesConfig } = require('./hyperliquid-research-lanes');
const { buildAuditStatus } = require('./hyperliquid-audit');
...
function evaluateMarketSignal({ candleRows, fundingRows, book, nowMs = Date.now() }, strategy = config) {
  ...
  if (validationReasons.length) {
    return {
      dataComplete: false,
      regimeRulePasses: false,
      regimeBlockers: validationReasons,
  ...
  const regimeChecks = [
    ...
    [r6 > rule.minReturnExclusive + 1e-12, `completed six-hour return is not above ${(rule.minReturnExclusive * 100).toFixed(0)}% for the regime stream`],
  ...
  const regimeBlockers = unique(regimeChecks.filter(([passed]) => !passed).map(([, reason]) => reason));
  const checks = [
    [!funding.error, funding.error],
    [r6 > rule.liveEntryMinReturnExclusive + 1e-12, `completed six-hour return is not above ${(rule.liveEntryMinReturnExclusive * 100).toFixed(0)}% for live entry`],
    [funding.rate !== null && funding.rate >= rule.minFundingRate, `post-signal funding settlement is below ${(rule.minFundingRate * 10_000).toFixed(1)} bps`],
  ...
  const regimePasses = regimeBlockers.length === 0;
  return {
    dataComplete: !funding.error && !bookState.error,
    regimeRulePasses: regimePasses,
    regimeBlockers,
    ...
    liveVsSignal,
    signalAgeMs,
    riskTier: strong ? 'strong_r6' : regimePasses ? 'virtual_regime_only' : 'not_qualified',
...
function accountPerformance(accountValueUsd, ledgerRows, ledgerError = null, strategy = config) {
  const accounting = strategy && strategy.accounting;
  const baselineEquityUsd = finiteNumber(accounting && accounting.baselineEquityUsd);
  const cashFlowCutoffMs = finiteNumber(accounting && accounting.cashFlowCutoffMs);
  const base = {
    status: 'unavailable',
    baselineEquityUsd,
    baselineLabel: accounting && accounting.baselineLabel ? String(accounting.baselineLabel) : null,
    cashFlowCutoffMs,
    cashFlowCutoffAt: accounting && accounting.cashFlowCutoffAt ? String(accounting.cashFlowCutoffAt) : null,
    subsequentCashFlowCount: null,
    lifetimeTradingPnlUsd: null,
    lifetimeTradingReturnPct: null,
    blocker: null
  };
  if (baselineEquityUsd === null || baselineEquityUsd <= 0 || cashFlowCutoffMs === null) {
    return { ...base, blocker: 'strategy accounting baseline is missing or invalid' };
  }
  if (ledgerError) return { ...base, blocker: `cash-flow source unavailable: ${ledgerError}` };
  if (!Array.isArray(ledgerRows)) return { ...base, blocker: 'cash-flow source is malformed' };
  const subsequent = ledgerRows.filter((row) => (
    row && finiteNumber(row.time) !== null && Number(row.time) > cashFlowCutoffMs
  ));
  if (subsequent.length) {
    return {
      ...base,
      status: 'cash_flow_reconciliation_required',
      subsequentCashFlowCount: subsequent.length,
      blocker: 'a deposit, withdrawal, or transfer occurred after the accounting baseline'
    };
  }
  const equity = finiteNumber(accountValueUsd);
  if (equity === null) return { ...base, subsequentCashFlowCount: 0, blocker: 'account equity is unavailable' };
  const pnl = equity - baselineEquityUsd;
  return {
    ...base,
    status: 'reconciled',
    subsequentCashFlowCount: 0,
    lifetimeTradingPnlUsd: pnl,
    lifetimeTradingReturnPct: (pnl / baselineEquityUsd) * 100,
    blocker: null
  };
}
...
async function buildPublicStatus({
  ...
  researchSnapshot = researchLanesConfig
} = {}) {
  ...
  const requests = {
    ...
    cashflows: {
      ...
      user,
      startTime: config.accounting.cashFlowCutoffMs + 1,
      endTime: nowMs
    },
  ...
  for (const name of ['candles', 'funding']) {
    if (sourceErrors[name]) signal.regimeBlockers = unique([...(signal.regimeBlockers || []), `${name} source unavailable`]);
  }
  signal.marketRulePasses = signal.blockers.length === 0;
  signal.regimeRulePasses = (signal.regimeBlockers || []).length === 0;
  ...
  return {
    ...
    strategy: config,
    audit: buildAuditStatus(undefined, nowMs),
    researchLanes: buildResearchLanesStatus(researchSnapshot, nowMs),
    account: {
      address: user,
