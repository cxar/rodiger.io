...
const KNOWN_STATUSES = new Set([
  ...
  'rejected_prelaunch_integrity_review',
...
function validateEvidence(evidence, status, prefix) {
  ...
  if (status === 'rejected_prelaunch_integrity_review') {
    if (
      ...
      throw new Error(`${prefix} rejected prelaunch lane must be quarantined with zero evidence`);
    }
  }
...
function validateLane(lane, index) {
  ...
  if (
    QUARANTINED_STATUSES.has(lane.operationalStatus) !== lane.quarantined
  ) {
